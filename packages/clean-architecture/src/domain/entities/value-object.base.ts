...
export abstract class ValueObject<Props> {
  ...
  #validateProps(props: ValueObjectProps<Props>) {
    const isEmptyObject = props && typeof props === "object" && Object.keys(props).length === 0;
    const isEmptyPrimitive =
      this.#isDomainPrimitive(props) && (props.value === null || props.value === undefined);

    if (!props || isEmptyObject || isEmptyPrimitive)
